      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i = 0; i < ptsx.size(); ++i) {
            double sht_x = ptsx[i] - px;
            double sht_y = ptsy[i] - py;
            ptsx[i] = sht_x * cos(0 - psi) - sht_y * sin(0 - psi);
            ptsy[i] = sht_x * sin(0 - psi) + sht_y * cos(0 - psi);
          }
          ...
          *
          */
          std::cout << "main loop solve start: " << state[0] << std::endl;

          auto vars = mpc.Solve(state, coeffs);

          std::cout << "main loop solve done: " << vars[0] << std::endl;
